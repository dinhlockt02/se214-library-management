...
func TestLogin(t *testing.T) {
	...
	t.Run("it should return a valid token if login info is correct", func(t *testing.T) {
		...
		mockId := entity.NewID()
		mockThuThuUsecase.On("GetThuThuByEmail", mockEmail).Return(&entity.ThuThu{
			MaThuThu: &mockId,
			Email:    mockEmail,
	...
	t.Run("it should return a error if find no thu thu", func(t *testing.T) {
		...
		mockEmail := "mock@email.com"
		mockPassword := "mock-password"
		mockThuThuNotFoundError := `mock-thu-thu-not-found-error`
		mockThuThuUsecase.On("GetThuThuByEmail", mockEmail).Return((*entity.ThuThu)(nil), errors.New(mockThuThuNotFoundError)).Once()
		rs, err := authService.Login(mockEmail, mockPassword)
		assert.Nil(t, rs)
		assert.Equal(t, mockThuThuNotFoundError, err.Error())
