...
func (mock *MockThuThuUsecase) CreateThuThu(name string, ngaySinh *time.Time, email string, phoneNumber string, password string) (*entity.ThuThu, error) {
	args := mock.Called(name, ngaySinh, email, phoneNumber, password)
	return args.Get(0).(*entity.ThuThu), args.Error(1)
}
